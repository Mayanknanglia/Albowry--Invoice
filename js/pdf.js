...
function generateInvoicePDF(invoiceId, preview = false) {
    ...
    drawCell(settings.companyName.toUpperCase(), pageW / 2, y, {
        bold: true, size: 16, align: 'center', color: [26, 58, 92]
    });
    y += 5.5;
    ...
    drawCell(`TRN: ${settings.trn || 'Not Set'}`, pageW / 2, y, {
        bold: true, size: 10.5, align: 'center'
    });
    y += 5;
    hLine(y);
    ...
    let leftY = y + 7;
    drawCell('Buyer (Bill to):', M + 3, leftY, { bold: true, size: 10.5 });
    leftY += 6;
    drawCell(invoice.customerName || 'N/A', M + 3, leftY, { bold: true, size: 12.5 });
    leftY += 6;
    ...
    invoice.items.forEach((item, idx) => {
        ...
        drawCell(formatNum(item.amount), col.amount.x + col.amount.w - 3, y + 5.5, {
    ...
    drawCell('Total Qty:', col.desc.x + col.desc.w - 3, y, {
        bold: true, size: 9, align: 'right', color: [100, 100, 100]
    });
    drawCell(formatNum(totalQtyCount), col.qty.x + col.qty.w / 2, y, {
    ...
    if (invoice.discount > 0) {
        ...
        drawCell('- ' + formatNum(invoice.discount), valueAlign, y, {
            bold: true, size: 10.5, align: 'right'
        });
        y += 5.5;
    }
    if (invoice.vatEnabled) {
        drawCell(`OUTPUT VAT @ ${invoice.vatRate}%`, labelAlign, y, {
            bold: true, size: 10.5, align: 'right'
        });
        drawCell(formatNum(invoice.vatAmount), valueAlign, y, {
            bold: true, size: 10.5, align: 'right'
        });
        y += 5.5;
    }
    if (invoice.roundOff > 0) {
        ...
        drawCell(`${sign} ${formatNum(invoice.roundOff)}`, valueAlign, y, {
    ...
    doc.rect(M, y, W, 10, 'F');
    y += 6.5;
    drawCell('GRAND TOTAL', labelAlign, y, {
        bold: true, size: 12, align: 'right', color: [26, 58, 92]
    });
    drawCell(`AED ${formatNum(invoice.grandTotal)}`, valueAlign, y, {
        bold: true, size: 13, align: 'right', color: [26, 58, 92]
    });
    y += 3.5;
    ...
    y += 6.5;
    drawCell(numberToWords(invoice.grandTotal), M + 3, y, {
        bold: true, size: 11, color: [26, 58, 92]
    });
    y += 4.5;
    hLine(y);
    ...
    const bx = M + 3;
    const by = M + 30;
    drawCell('A/c Holder:', bx, bankY, { bold: true, size: 9.5 });
    drawCell(settings.companyName, by, bankY, { bold: true, size: 9.5 });
    bankY += 4.5;
    drawCell('Bank Name:', bx, bankY, { bold: true, size: 9.5 });
    drawCell(settings.bankName || '-', by, bankY, { size: 9.5 });
    bankY += 4.5;
    drawCell('A/c No.:', bx, bankY, { bold: true, size: 9.5 });
    drawCell(settings.bankAccount || '-', by, bankY, { bold: true, size: 9.5 });
    bankY += 4.5;
    ...
    drawCell('Authorised Signatory', M + W - 3, boxBottomY - 3.5, {
        bold: true, size: 9.5, align: 'right'
    });
